...
class PipelineBuilder:
    ...
    def evaluate_pipeline(self, X, y):
        ...
        # Collect SNP pairs from each epi node after fitting
        epi_pairs = [(epi_node.snp1_name, epi_node.logical_operator, epi_node.snp2_name) for epi_node in self.pipeline.epi_branches]
        self.pipeline.epi_pairs = epi_pairs
    ...
    def score(self, X, y):
        ...
        #feature_count = self.fitted_pipeline.get_selector_node().get_feature_count()
        #feature_count = len(self.fitted_pipeline.get_feature_names_out())
    ...
    def get_final_epi_pairs(self):
        ...
        if selector_node is None:
            raise ValueError("Selector node is not found in the pipeline.")
