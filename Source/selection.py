...
class Selection:
    ...
    def NonDominatedSorting(self, obj_scores: npt.NDArray[np.float32]) -> Tuple[List[List[np.uint16]],npt.NDArray[np.uint16]]:
        """
        ...
        Returns:
        list of list of int: Each sublist contains the indices of solutions in the corresponding Pareto front.
        """
        ...
        for p in range(pop_size):
            for q in range(pop_size):
                if self.Dominates(obj_scores[p], obj_scores[q]):
                    dominated_solutions[p].append(q)
                elif self.Dominates(obj_scores[q], obj_scores[p]):
                    domination_count[p] += 1

            if domination_count[p] == 0:
                rank[p] = 0
                fronts[0].append(np.uint16(p))
        ...
        while len(fronts[i]) > 0:
            ...
            for p in fronts[i]:
                for q in dominated_solutions[p]:
                    ...
                    assert domination_count[q] >= 0 #check that it's always non-negative
                    if domination_count[q] == 0:
                        rank[q] = i + 1
                        next_front.append(np.uint16(q))
            i += 1
            fronts.append(next_front)

        fronts.pop()
        print('internal fronts:', fronts)
        print('internal rank:', rank)
        return fronts, rank

    def CrowdingDistance(self, obj_scores: npt.NDArray[np.float32], count: np.int32) -> npt.NDArray[np.float32]:
        """
    ...
    def Dominates(self, solution1: npt.NDArray[np.float32], solution2: npt.NDArray[np.float32]):
        """
    ...
    def NonDominatedBinaryTournament(self, ranks: npt.NDArray[np.uint16], distances: npt.NDArray[np.float32], rng: np.random.Generator) -> np.uint16:
    ...
    def NonDominatedTruncate(self, fronts: List[npt.NDArray[np.uint16]], distances: npt.NDArray[np.float32], N: np.uint16) -> npt.NDArray[np.uint16]:
